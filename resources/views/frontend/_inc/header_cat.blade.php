...
@if (Session::has('cart'))
    @foreach (Session::get('cart') as $key => $cart)
        ...
        @endphp
        <div class="entry">
            <div class="entry-thumb"><a href="{{ route('frontend.product', $cart['slug']) }}"><img src="{{ asset($cart['photo']) }}" alt="Product"></a></div>
            ...
            <div class="entry-delete">
                <a href="{{ route('frontend.cart.destroy', $key) }}">
                    <i class="icon-x"></i>
                </a>
            </div>
        </div>
    ...
    <div class="d-flex justify-content-between">
        <div class="w-50 d-block">
            <a class="btn btn-secondary btn-sm  mb-0" href="{{ route('frontend.cart') }}">{{ __('Go to Cart') }}</a>
        </div>
        <div class="w-50 d-block text-end">
            <a class="btn btn-primary btn-sm  mb-0" href="{{ route('frontend.checkout.billing') }}">{{ __('Checkout') }}</a>
        </div>
    @else
        {{ __('Cart Is Empty.') }}
@endif
