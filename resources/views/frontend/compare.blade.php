            ...
            <div class="card-body">
                <div class="comparison-table">
                    <table class="table table-bordered">
                                ...
                                @if (count($items) != 1)
                                    <tr>
                                        <td>
                                            ...
                                            <p>
                                                <b>{{ __('Brand') }}</b> : {{ $items[0]->brand ? $items[0]->brand->name : 'Not Available.' }} <br> <b>{{ __('Price') }}</b> : {{ PriceHelper::grandCurrencyPrice($items[0]) }}
                                            </p>
                                            <hr>
                                            <h6 class="mt-2">
                                                {{ $items[1]->name }}
                                            ...
                                            <p>
                                                <b>{{ __('Brand') }}</b> : {{ $items[1]->brand ? $items[1]->brand->name : 'Not Available.' }} <br> <b>{{ __('Price') }}</b> : {{ PriceHelper::grandCurrencyPrice($items[1]) }}
                                            </p>
                                        </td>
                                        @foreach ($items as $item)
                                            <td>
                                                <div class="comparison-item">
                                                    <span class="remove-item compare_remove" data-href="{{ route('frontend.compare.remove', $item->id) }}"><i class="icon-x"></i></span><a class="comparison-item-thumb" href="{{ route('frontend.product', $item->slug) }}"><img src="{{ asset($item->thumbnail) }}" alt="Image"></a>
                                                    <a class="comparison-item-title" href="{{ route('frontend.product', $item->slug) }}">{{ $item->name }}</a>
                        ...
                        @else
                            <tr>
                                <td class="text-center"><strong>{{ __('No Products Found.') }}</strong></td>
                            </tr>
                            @endif
                        </tbody>
                    </table>
