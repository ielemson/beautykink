        ...
        <div class="hero-area3">
            ...
            <div class="heroarea-slider owl-carousel">
                @foreach ($sliders as $slider)
                    <div class="item" style="background: url('{{ asset($slider->photo) }}')">
                        <div class="container">
                            <div class="row">
                                ...
                                <div class="col-lg-6 order-first order-lg-last">
                                    <div class="layer-4">
                                        <div class="right-img">
                                            <img class="img-fluid full-img" src="{{ asset($slider->logo) }}" alt="">
                                        </div>
                                    </div>
                                </div>
                            </div>
                ...
                <div class="row ">
                    <div class="col-md-4">
                        <a href="{{ $banner_first['firsturl1'] }}" class="genius-banner">
                            <img class="lazy" src="{{ $setting->loader ? asset($setting->loader) : asset('frontend/images/ajax_loader.gif')}}" data-src="{{ asset($banner_first['img1']) }}" alt="">
                        </a>
                    </div>
                    <div class="col-md-4">
                        <a href="{{ $banner_first['firsturl2'] }}" class="genius-banner">
                            <img class="lazy" src="{{ $setting->loader ? asset($setting->loader) : asset('frontend/images/ajax_loader.gif')}}" data-src="{{ asset($banner_first['img2']) }}" alt="">
                        </a>
                    </div>
                    <div class="col-md-4">
                        <a href="{{ $banner_first['firsturl3'] }}" class="genius-banner">
                            <img class="lazy" src="{{ $setting->loader ? asset($setting->loader) : asset('frontend/images/ajax_loader.gif')}}" data-src="{{ asset($banner_first['img3']) }}" alt="">
                        </a>
                    </div>
                </div>
            </div>
                        ...
                        <div class="popular-category theme3">
                            <div class="links">
                                @foreach ($popular_categories as $key => $popular_categorie)
                                    <a class="category_get {{ $loop->first ? 'active' : '' }}" data-target="popular_category_view" data-href="{{ route('frontend.popular.category', [$popular_categorie->slug, 'popular_category', 'slider']) }}" href="javascript:;" class="{{ $loop->first ? 'active' : '' }}">{{ $popular_categorie->name }}</a>
                                @endforeach
                            </div>
                        </div>
                    </div>
                                ...
                                <div class="slider-item">
                                    <div class="product-card">
                                        <a class="product-thumb"
                                            ...
                                            @endif
                                            <img class="lazy" src="{{ $setting->loader ? asset($setting->loader) : asset('frontend/images/ajax_loader.gif')}}" data-src="{{ asset($popular_category_item->thumbnail) }}" alt="Product">
                                        </a>
                                        <div class="product-card-body">
                                            <div class="product-category">
                                                <a href="{{ route('frontend.catalog') . '?category=' . $popular_category_item->category->slug }}">{{ $popular_category_item->category->name }}</a>
                                            </div>
                                            <h3 class="product-title">
                                                <a href="{{ route('frontend.product', $popular_category_item->slug) }}">
                                            ...
                                            <div class="rating-stars">
                                                <i class="far fa-star filled"></i><i class="far fa-star filled"></i>
                                                <i class="far fa-star filled"></i><i class="far fa-star filled"></i>
                                        ...
                                        <div class="product-button-group">
                                            <a class="product-button wishlist_store" href="{{ route('user.wishlist.store', $popular_category_item->id) }}"><i class="icon-heart"></i><span>{{ __('Wishlist') }}</span></a>
                                            <a data-target="{{ route('frontend.compare.product', $popular_category_item->id) }}" class="product-button product_compare" href="javascript:;"><i class="icon-repeat"></i><span>{{ __('Compare') }}</span></a>

                                            @if ($popular_category_item->stock != 0)
                                                <a class="product-button add_to_single_cart" data-target="{{ $popular_category_item->id }}" href="javascript:;"><i class="icon-shopping-cart"></i><span>{{ __('To Cart') }}</span></a>
                                            @else
                                        ...
                                        <div class="slider-item">
                                            <div class="product-card ">
                                                ...
                                                <div class="product-card-inner">
                                                    ...
                                                    <div class="product-button-group">
                                                        <a class="product-button" href="{{ route('user.wishlist.store', $item->id) }}"><i class="icon-heart"></i><span>{{ __('Wishlist') }}</span></a>
                                                        <a data-target="{{ route('frontend.compare.product', $item->id) }}" class="product-button product_compare" href="javascript:;"><i class="icon-repeat"></i><span>{{ __('Compare') }}</span></a>
                                                        @if ($item->is_stock())
                                                            <a class="product-button add_to_single_cart" data-target="{{ $item->id }}" href="javascript:;"><i class="icon-shopping-cart"></i><span>{{ __('To Cart') }}</span></a>
                                                        @else
            ...
            <div class="container">
                <div class="row">
                    <div class="col-lg-12">
                        ...
                        <div class="popular-category theme3">
                            <div class="links">
                                <a data-href="{{ route('frontend.get.product', 'feature') }}" data-target="type_product_view" href="javascript:;" class="product_get active">{{ __('Featured') }}</a>
                                <a data-href="{{ route('frontend.get.product', 'best') }}" data-target="type_product_view" class="product_get" href="javascript:;">{{ __('Best Seller') }}</a>
                                <a data-href="{{ route('frontend.get.product', 'top') }}" data-target="type_product_view" class="product_get" href="javascript:;">{{ __('Top Rated') }}</a>
                                <a data-href="{{ route('frontend.get.product', 'new') }}" data-target="type_product_view" class="product_get" href="javascript:;">{{ __('New Product') }}</a>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="col-lg-12">
                        <div class="home-blog-slider owl-carousel">
                            @foreach ($posts as $post)
                                <div class="slider-item">
                                    <div class="blog-post">
                                        <a class="post-thumb" href="{{ route('frontend.blog.details', $post->slug) }}"><img class="lazy" src="{{ $setting->loader ? asset($setting->loader) : asset('frontend/images/ajax_loader.gif')}}" data-src="{{ asset(json_decode($post->photo, true)[array_key_first(json_decode($post->photo, true))]) }}" alt="Blog Post"></a>
                                        <div class="post-body">
                                            <h3 class="post-title">
