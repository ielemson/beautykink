                                    ...
                                    @foreach (explode(',', $post->tags) as $tag)
                                        @if ($loop->last)
                                            <a class="text-sm text-muted navi-link"
                                                href="{{ route('frontend.blog') . '?tag=' . $tag }}">{{ $tag }}</a>
                                        @else
                                            <a class="text-sm text-muted navi-link"
                                                href="{{ route('frontend.blog') . '?tag=' . $tag }}">{{ $tag }}</a>,
                                        @endif
                            ...
                            <div class="widget widget-featured-posts">
                                <div class="entry">
                                    <div class="entry-thumb">
                                        <a href="{{ route('frontend.blog.details', $like_post->slug) }}">
                                            <img src="{{ asset(json_decode($like_post->photo, true)[array_key_first(json_decode($like_post->photo, true))]) }}" alt="Post">
                                        </a>
                                    </div>
                                    <div class="entry-content">
                                        ...
                                        <span class="entry-meta">{{ __('by') }} {{ __('Admin') }}</span>
                                    </div>
                                </div>
                            </div>
                        @endforeach
            ...
            <div class="col-xl-3 col-lg-4 order-lg-1">
                ...
                <aside class="sidebar sidebar-offcanvas position-left">
                    ...
                    <section class="widget">
                        <form action="{{ route('frontend.blog') }}" class="input-group form-group" method="get">
                    ...
                    <section class="widget widget-categories card rounded p-4 mt-n3">
                        ...
                        <ul>
                            @foreach ($categories as $category)
                                <li>
                                    <a href="{{ route('frontend.blog') . '?category=' . $category->slug }}">{{ $category->name }}</a><span>{{ $category->posts_count }}</span>
                                </li>
                            @endforeach
                    ...
                    <section class="widget widget-featured-posts card rounded p-4">
                        ...
                        @foreach ($posts as $recent)
                            <div class="entry">
                                <div class="entry-thumb"><a href="{{ route('frontend.blog.details', $recent->slug) }}"><img
                                            src="{{ asset(json_decode($recent->photo, true)[array_key_first(json_decode($recent->photo, true))]) }}"
                                            alt="Post"></a></div>
                                <div class="entry-content">
                                    <h4 class="entry-title">
                                        <a href="{{ route('frontend.blog.details', $recent->slug) }}">
                    ...
                    <section class="widget widget-featured-posts card rounded p-4">
                        ...
                        <div>
                            @foreach ($tags as $tag)
                                <a class="tag" href="{{ route('frontend.blog') . '?tag=' . $tag }}">{{ $tag }}</a>
                            @endforeach
                        </div>
                    </section>
                </aside>
