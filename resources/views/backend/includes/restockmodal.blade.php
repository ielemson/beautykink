...
// Set Stock Limit Ajax Call
$('.set_stock_limit').on('click', function() {
    //   var id = $(this).attr("data-id");
        $.confirm({
            title: 'Set Stock Limit',
            theme: 'bootstrap',
            closeIcon: true,
            animation: 'scale',
            type: 'blue',
            content: '' +
                '<form action="" class="formName">' +
                '<div class="form-group">' +
                '<input type="number" min="1" placeholder="Set Stock Limit" class="item_stock_limit form-control" required />' +
                '</div>' +
                '</form>',
            buttons: {
                formSubmit: {
                    ...
                    btnClass: 'btn-blue',
                    action: function() {
                        var item_stock_limit = this.$content.find('.item_stock_limit').val();
                        if (!item_stock_limit) {
                            $.alert('provide a valid number');
                            return false;
                        }
                        ...
                        $.ajax({
                        type:'POST',
                        url:"{{ route('backend.item.restock.limit') }}",
                        data:{ item_stock_limit:item_stock_limit},
                        beforeSend: function () { // Before we send the request, remove the .hidden class from the spinner and default to inline-block.
                      $('.spinner').html(`         
                      <span class="float-left">Save <i class="fas fa-spinner fa-spin "></i></span>
                        `)
                         },
                        success:function(data){
                            console.log(data)
