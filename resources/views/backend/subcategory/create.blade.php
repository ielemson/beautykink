        ...
        <div class="row">
          ...
          <div class="col-md-12">
            ...
            <div class="card">
              <!-- /.card-header -->
              <!-- form start -->
              <form action="{{ route('backend.subcategory.store') }}" method="POST" id="quickForm" enctype="multipart/form-data">
                @csrf
                <div class="card-body row">
                  ...
                  <div class="form-group  col-md-12">
                    ...
                    <select name="category_id" id="category_id" class="form-control select2">
                        @foreach ($categories as $cat)
                            <option value="{{ $cat->id }}" {{ old('category_id') && old('category_id') == $cat->id ? 'selected' : '' }}>{{ $cat->name }}</option>
                        @endforeach
                    </select>
                  </div>
                  ...
                  <div class="form-group  col-md-12">
                    ...
                    <input type="text" name="name" class="form-control item-name" id="name" placeholder="{{ __('Enter Name') }}" value="{{ old('slug') }}">
                  </div>
