  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1>{{ __('Update License Product') }}</h1>
          </div>
        </div>
      </div><!-- /.container-fluid -->
    </section>
            ...
            <div class="card">
              ...
              <form action="{{ route('backend.item.update', $item->id) }}" method="POST" id="quickForm" enctype="multipart/form-data">
                ...
                <div class="card-body row">
                  ...
                  <div class="form-group col-md-6">
                    ...
                    <div class="input-group">
                        <div class="custom-file">
                          ...
                          <label class="custom-file-label" for="file">{{ __('Upload Image...') }}</label>
                        </div>
                      </div>
                  </div>
                  ...
                  <div class="form-group  col-md-6 view_file {{ $item->file_type == 'file' ? '' : 'd-none' }}">
                    <label for="file">{{ __('File') }} * <span class="text-warning">({{__('File type must be zip')}})</span>@if (file_exists($item->file)) <a download href="{{asset($item->file)}}">{{__('Download File')}}</a> @endif</label>
                    <input type="file" name="file" class="form-control p-1" id="file" >

                  </div>

                  <div class="form-group  col-md-6 view_link {{ $item->file_type == 'link' ? '' : 'd-none' }}">
                    <label for="link">{{ __('Link') }} *</label>
                    <input type="text" name="link" class="form-control p-1" id="link" placeholder="{{__('Enter Link')}}" value="{{ $item->link }}" placeholder="{{ __('Enter Link') }}">
                  </div>
                  ...
                  <div class="form-group  col-md-6">
                    ...
                    <div class="input-group mb-3">
                        ...
                        <input type="number" name="previous_price" id="previous_price" min="0" step="0.01" class="form-control" placeholder="{{ __('Enter Previous Price') }}" value="{{ round($item->previous_price * $curr->value, 2) }}">
                    </div>
                  </div>
                  ...
                  <div class="form-group col-md-12">
                    <label class="" for="">{{ __('Add License') }} *</label>
                  </div>

                  <div class="col-md-12" id="license-section">
                      @if (!empty($license_name))
                        @foreach (array_combine($license_name,$license_key) as  $name => $key)
                            <div class="row">
                                <div class="form-group  col-md-5">
                                    <input type="text" name='license_name[]' class='form-control' placeholder="{{ __('License Name') }}" value="{{ $name }}">
                                </div>

                                <!-- /.col-lg-6 -->
                                <div class="form-group  col-md-6">
                                    <input type="text" name='license_key[]' class='form-control' placeholder="{{ __('License Key') }}" value="{{ $key }}">
                                </div>
                                <!-- /.col-lg-6 -->

                                <div class="form-group  col-md-1">
                                    @if ($loop->first)
                                        <button data-text="{{ __('License Name') }}" data-text1="{{ __('License Key') }}" type="button" class="btn btn-info btn-md add-license"><i class="fa fa-plus"></i></button>
                                    @else
                      ...
                      @else
                        <div class="row">
                            <div class="form-group  col-md-5">
                                <input type="text" name='license_name[]' class='form-control' placeholder="{{ __('License Name') }}" value="">
                            </div>

                            <!-- /.col-lg-6 -->
                            <div class="form-group  col-md-6">
                                <input type="text" name='license_key[]' class='form-control' placeholder="{{ __('License Key') }}" value="">
                            </div>
                            <!-- /.col-lg-6 -->
                  ...
                  <div class="form-group  col-md-6">
                    ...
                    <input type="text" name='meta_keywords' id="meta_keywords" class='form-control tags' placeholder="{{ __('Enter Meta Keywords') }}" value="{{ $item->meta_keywords }}">
                  </div>

                  <div class="form-group  col-md-12">
                    <label for="meta_descriptions">{{ __('Meta Description') }}</label>
                    <textarea name='meta_descriptions' id="meta_descriptions" class='form-control' placeholder="{{ __('Enter Meta Description') }}" rows="5">{{ $item->meta_description }}</textarea>
                  </div>
