...
class PriceHelper
{
    ...
    public static function licenseQtyDecrease($cart)
    {
        foreach ($cart as $item_id => $item) {
            if ($item['item_type'] == 'license') {
                $item = Item::findOrFail(PriceHelper::getItemId($item_id));
                $license_key_new = json_decode($item_id->license_key, true);
                ...
                $item_id->license_name = json_decode($license_name_new, true);
                $item_id->license_key = json_decode($license_key_new, true);
                $item->update();
            }
        }
    }
