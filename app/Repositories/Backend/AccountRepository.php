...
class AccountRepository
{
    ...
    */
    public function updateProfile($request)
    {
        ...
        if ($file = $request->file('photo')) {
            $input['photo'] = ImageHelper::handleUpdatedUploadedImage($file, 'uploads/profile', $data, 'photo');
        }
        $data->update($input);
    }
    ...
    */
    ...
    public function getCurrentMonthProductSale()
    {
        ...
        $frist_day = Carbon::parse($implode);
        $orders = Order::whereOrderStatus('Delivered')->whereDate('created_at', '>=', $frist_day)->whereDate('created_at', '<=', $current_date)->get();
        $total_items_qty = 0;
        foreach ($orders as $order) {
            $cart = json_decode($order->cart, true);
            foreach ($cart as $item) {
    ...
    public function getMonthEarning()
    {
        ...
        $frist_day = Carbon::parse($implode);
        $total = 0;
        $orders = Order::whereOrderStatus('Delivered')->whereDate('created_at', '>=', $frist_day)->where('created_at', '<=', $current_date)->get();
    ...
    public function getYearEarning()
    {
        ...
        $year = date('Y-m-d', strtotime('today - 365 days'));
        $total = 0;
        $orders = Order::whereOrderStatus('Delivered')->whereDate('created_at', '>=', $year)->where('created_at', '<=', $current_date)->get();
